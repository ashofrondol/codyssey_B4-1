...
| 항목         | 링크                                                     |
| ------------ | -------------------------------------------------------- |
| 배포 사이트  | `https://JungSaeYoung.github.io/<repo-name>/` (배포 후 갱신) |
| GitHub 저장소 | `https://github.com/JungSaeYoung/<repo-name>` (배포 후 갱신) |

> ⚠️ 배포 전입니다. GitHub Pages 배포가 완료되면 위 URL을 실제 값으로 교체해 주세요.
...
|---|-----------|---------------|-------------|
| 1 | `Theme.state` | `themeToggle` 클릭 | `html[data-theme]` 변경 → 전체 색상 전환 |
| 2 | `Projects.state` (`status`, `repos`, `activeLang`) | `fetch` / 필터 클릭 / 재시도 | 로딩 · 성공 · 에러 · 빈 상태 UI |
| 3 | Contact 폼 (`has-error` 클래스) | `blur` / `submit` | 에러 메시지 표시/숨김, 성공 메시지 |
| 4 | (보너스) `activeLang` | 필터 버튼 클릭 | 언어별 카드 목록 변경 |
...
├── css/
│   └── style.css      # 변수, 라이트/다크, 반응형
├── js/
│   └── main.js        # 이벤트 / 상태 / API / 폼
├── images/            # (사용 시) 이미지 자산
└── README.md
```
...
> **본인 데이터로 바꾸려면** `js/main.js` 의 `CONFIG.GITHUB_USERNAME` 값을 본인 GitHub 아이디로 변경하세요.
...
`js/main.js` 상단의 `CONFIG` 객체에서 다음 값을 변경할 수 있습니다.

| Key | 의미 | 기본값 |
|-----|------|--------|
| `GITHUB_USERNAME` | GitHub API에 사용할 아이디 | `JungSaeYoung` |
